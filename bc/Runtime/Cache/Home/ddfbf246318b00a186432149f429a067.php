...
<form method="post" action="/index.php/Home/Company/editItem" name="f1" enctype="multipart/form-data">
<input name="id" value="<?php echo ($item["id"]); ?>" style="display:none"/>
<input name="name" placeholder="名称" value="<?php echo (stripslashes($item["name"])); ?>"/>
<input name="cn_name" placeholder="别名" value="<?php echo ($item["cn_name"]); ?>"/>
...
<input name="since" placeholder="创建年份" value="<?php echo ((isset($item["since"]) && ($item["since"] !== ""))?($item["since"]):'2000'); ?>"/>
 ...
 <script id="container" name="introduction" type="text/plain">
        <?php echo (stripslashes(htmlspecialchars_decode($item["introduction"]))); ?>
    </script>
