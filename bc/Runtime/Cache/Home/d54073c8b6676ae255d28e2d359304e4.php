...
<body>
    ...
    <div class="c-home">
        ...
        <a href="/index.php/Home/bestforbaby/viewitem/id/<?php echo ($item["id"]); ?>">查看物品</a>
    </div>
    ...
    <form method="post" action="/index.php/Home/Instruction/<?php echo ($action); ?>" name="f1" enctype="multipart/form-data">
        <?php if(!empty($item)): ?><input name="item_id" value="<?php echo ($item["item_id"]); ?>" style="display:none;"/>
        <input name="id" value="<?php echo ($item["id"]); ?>" style="display:none;"/><?php endif; ?>
        <select name="title_id">
	        <option>选择题目</option>
            <?php if(!empty($item)): ?><option value="<?php echo ($item["title_id"]); ?>" selected><?php echo ($item["title"]); ?></option><!--说明ID--><?php endif; ?>
        ...
        <?php if(!empty($item)): echo (htmlspecialchars_decode($item["content"])); endif; ?>
        </script>
